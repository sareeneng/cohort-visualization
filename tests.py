...
class TestPathFinding(unittest.TestCase):
	...
	def test_two_tables(self):
		x = self.db.struct.find_paths_between_tables('A', 'F')
		self.assertEqual(len(x), 3)
		self.assertIn(['A', 'D', 'C', 'F'], x)
		self.assertIn(['A', 'C', 'F'], x)
		self.assertIn(['A', 'B', 'E', 'F'], x)

		x = self.db.struct.find_paths_between_tables('B', 'F')
		self.assertEqual(len(x), 3)
		...
		x = self.db.struct.find_paths_between_tables('A', 'C')
		self.assertEqual([['A', 'C']], x)

		x = self.db.struct.find_paths_between_tables('A', 'A')
		self.assertEqual(['A'], x)

		x = self.db.struct.find_paths_between_tables('B', 'E')
		self.assertEqual([['B', 'E']], x)

		x = self.db.struct.find_paths_between_tables('E', 'B')
		self.assertEqual([], x)

	def test_multi(self):
		# test back-tracking with multi_tables path-finding
		x = self.db.struct.find_paths_multi_tables(['A', 'D', 'C', 'F'])
		self.assertEqual(len(x), 2)
		self.assertIn(['A', 'D', 'C', 'F'], x)
		self.assertIn(['A', 'C', 'D', 'C', 'F'], x)

		x = self.db.struct.find_paths_multi_tables(['A', 'B', 'D', 'E'])
		self.assertEqual([], x)

		x = self.db.struct.find_paths_multi_tables(['D', 'C', 'F'])
		self.assertEqual(len(x), 2)
		self.assertIn(['D', 'C', 'F'], x)
		self.assertIn(['C', 'D', 'C', 'F'], x)

		x = self.db.struct.find_paths_multi_tables(['D', 'C', 'F'], fix_first=True)
		self.assertEqual([['D', 'C', 'F']], x)

		colx = self.db.struct.find_col_idx_by_table('A', 'col1')
		coly = self.db.struct.find_col_idx_by_table('B', 'col4')  # in tables B, E
		colz = self.db.struct.find_col_idx_by_table('F', 'col8')  # only in table F

		x = self.db.struct.find_paths_multi_columns([colx, coly, colz])
		...
		x = self.db.struct.find_paths_multi_columns([colx, coly, colz], fix_first=True)
		self.assertEqual(len(x), 3)
